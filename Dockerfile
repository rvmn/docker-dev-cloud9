...
RUN source ~/.nvm/nvm.sh && echo "source ~/.nvm/nvm.sh" >> ~/.bashrc && source ~/.bashrc && var=$(nvm ls-remote | tail -1 | cut -d'v' -f 2) && echo "var=$(nvm ls-remote | tail -1 | cut -d'v' -f 2)" >> ~/.bashrc && source ~/.bashrc && echo "~/.nvm/v${var}/bin:${PATH}" >> ~/.bashrc && cat ~/.bashrc && source ~/.bashrc && nvm install $var && source ~/.bashrc && npm install -g sm && $(echo "~/.nvm/$var/lib/node_modules/sm/bin/sm install")

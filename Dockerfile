...
RUN var=v$(nvm ls-remote | tail -1 | cut -d'v' -f 2) && echo "/nvm/$var/bin:${PATH}" >> ~/.bashrc && cat ~/.bashrc && source ~/.bashrc && nvm install $var && nvm use $var && npm install -g sm && $(echo "/nvm/$var/lib/node_modules/sm/bin/sm install")
